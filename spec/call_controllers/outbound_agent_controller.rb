...
describe OutboundAgentController do
  ...
  subject { OutboundAgentController.new(mock_call) }

  describe "#grab_agent_object" do

    # before { subject.stub!(:grab_agent_object).and_return() }
    before { subject.agent = Agent.new agent_ext, "campaignTest" }
    before { subject.agent = Campaign.new "campaignTest" }

    it "generates a random number" do
      subject.agent.extension.should eq agent_ext
    end
  end
